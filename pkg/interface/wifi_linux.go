...
import (
	"bufio"
	"fmt"
	"github.com/paragor/paradrop80211/pkg/os_sugar"
	"regexp"
	"strconv"
	"strings"
)
...
func SetInterfaceChannel(iface string, channel int) error {
	if os_sugar.HasBinary("iw") {
		out, err := os_sugar.Exec("iw", []string{"dev", iface, "set", "channel", fmt.Sprintf("%d", channel)})
		if err != nil {
	...
	} else if os_sugar.HasBinary("iwconfig") {
		out, err := os_sugar.Exec("iwconfig", []string{iface, "channel", fmt.Sprintf("%d", channel)})
		if err != nil {
...
func iwlistSupportedFrequencies(iface string) ([]int, error) {
	out, err := os_sugar.Exec("iwlist", []string{iface, "freq"})
	if err != nil {
		return nil, err
	}
...
func iwSupportedFrequencies(iface string) ([]int, error) {
	...
	out, err := os_sugar.Exec("iw", []string{iface, "info"})
	if err != nil {
	...
	out, err = os_sugar.Exec("iw", []string{phyName, "info"})
	if err != nil {
...
func GetSupportedFrequencies(iface string) ([]int, error) {
	...
	} else if os_sugar.HasBinary("iwlist") {
